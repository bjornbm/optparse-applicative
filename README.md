...
The core type in optparse-applicative is the `Parser` type constructor,
which provides `Functor`, `Applicative`, and `Alternative` instances.

A value of type `Parser a` represents a specification for a set of options,
which, when parsed correctly, will yield a result of type `a`.

Those familiar with a parser combinator library like [parsec],
[attoparsec], or the json parser [aeson], should feel right at home
with optparse-applicative. Others should not worry, as the handful of
primitives and concepts required are described below.
...
it's just a matter of using `fmap` (or `(<$>)`) and `Applicative`'s apply
operator, `(<*>)`, to combine the two previously defined parsers:
...
these parsers with a choice operation:
...
The `progDesc`, `header`, and `footer` commands can be used to specify a brief
